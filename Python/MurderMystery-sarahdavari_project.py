...
def get_average_sentence_length(text):
    ...
    sentence_list2=[]
    length=[]
    number_of_words=float(0)
    for n in sentence_list:
        sentence_list2=n.split(" ")
        number_of_words+=(len(sentence_list2))
        sentence_number=len(sentence_list)
    ave=(number_of_words/sentence_number)
...
class TextSample:
    ...
    def __repr__(self):
        return "The Author's name is {}:The average sentence length is {}".format(self.author,self.average_sentence_length)     
...
def ngram_creator(text_list):
    ...
    for n in range(0,len(text_list)-1):
        pairwords= text_list[n]+" " + text_list[n+1]
        new_paired_list.append(pairwords)
...
print(percent_difference(10,11))
...
def find_text_similarity(note1, note2):
    sentence_length_difference = percent_difference(note1.average_sentence_length, note2.average_sentence_length)
    sentence_length_similarity = abs(1 - sentence_length_difference)
    word_count_similarity = frequency_comparison(note1.word_count_frequency, note2.word_count_frequency)
    ngram_similarity = frequency_comparison(note1.ngram_frequency, note2.ngram_frequency)
...
gress_similarity=(find_text_similarity(murderer_sample, gregg_sample))
all_similarity= {lily_sample.author:lily_similarity,myrtle_sample.author:myrtle_similarity,gregg_sample.author:gress_similarity}
...
print("{}'s text similarity to the murder letter is {}".format(lily_sample.author, find_text_similarity(murderer_sample, lily_sample)))
print("{}'s text similarity to the murder letter is {}".format(myrtle_sample.author, find_text_similarity(murderer_sample, myrtle_sample)))
print("{}'s text similarity to the murder letter is {}".format(gregg_sample.author, find_text_similarity(murderer_sample, gregg_sample)))
...
for keyy,similarity in all_similarity.items():
    sim=0
    if similarity>sim:
        sim=similarity
print ("{} is possibly Jay's killer based on the text similarity!".format(keyy, sim))
